...
export function girdLineEchartOptions(f = { id, x, nameX, right, y, name, unit, colors, rorate, legend, max, t }) {

    const { id, x = [], y = [], nameX, name = [], unit = '', colors = lineColor, rorate = false, legend = false, max = 0, t, right } = f;
    let chart = echarts.init(document.getElementById(id));
    let option = {
        tooltip: t ? t : tooltip,
        grid: right ? {
...
export function sanKeyOption(s) {
    ...
    const { id = '', f = [], fontSize = 12, hiddenLabel } = s
    ...
    let option = {
        ...
        series: {
            ...
            label: {
                ...
                fontSize: fontSize,
                formatter: '{b}: {c}',
                show: hiddenLabel ? false : true
            },
            nodeGap: hiddenLabel ? 4 : 16
        },
        tooltip: {
            trigger: 'item',
            triggerOn: 'mousemove'
