<template>
  <div class="home dc-body">
      ...
      <div class="item-box" style="padding: 0 0 0 15px; z-index: 2">
        ...
        <div class="chartbox jywl">
          ...
          <div v-show="isAgv">
            ...
            <div id="avg1" style="width: 100%; height: 100px" @click="showkxsjDialog"></div>
          </div>
          <div v-if="!isAgv">
            <div class="chart-tit">
              <span>收货箱数</span>
        ...
        <div class="chartbox nyhj" style="position: relative">
          ...
          >
            <el-carousel-item :key="4">
              ...
              <div id="nyhj4" style="width: 100%; height: 120px"></div>
            </el-carousel-item>
    ...
    <div class="as-dialog-f" v-show="bdcShow" @click="bdcShow=false">
    ...
    <div class="as-dialog-f" v-show="kxsjShow" @click="kxsjShow=false">
      <div class="as-dialog" @click.stop="()=>{}">
        <div class="header" style="position: absolute; right: 20px; top: 20px">
          <i class="el-icon-close" style="color: #fff; font-size: 40px" @click="kxsjShow = false"></i>
        </div>
        <div class="infotable-cs">
          <div
            class="box-tit"
          ...
          <div id="bdc5" style="width: 760px; height: 500px"></div>
        </div>
      </div>
    </div>
    <div class="as-dialog-f" v-show="handImgShow" @click="handImgShow=false">
      <div class="as-dialog" @click.stop="()=>{}">
...
export default {
  ...
  methods: {
    ...
    showkxsjDialog() {
      //   this.pieDialogTitle = "空闲时间占比";
      this.kxsjShow = true
      const { dataSum, dateSum, t, name } = this.dialogData
      if (!dateSum || dateSum.length == 0) return
      let data = dataSum
      let date = dateSum
      let option = { id: 'bdc5', name, x: date, y: [data], t }
    ...
    onChangeImg(floor) {
      ...
      //   停线记录
      let LB = floor == 1 ? 'YI-LOU' : 'ER-LOU'
      downtime_record({ ...this.key, rowNum: 2, lineBody: LB }).then(res => {
        if (res.data.success === true) {
    ...
    getFixRecord() {
      ...
      // 停线记录
      downtime_record({ ...this.key, rowNum: 2, lineBody: 'YI-ROU' }).then(res => {
        if (res.data.success === true) {
          this.fixData = res.data.data.map((x) => ({
            ...x,
            sTime: moment(x.sTime).format("MM-DD HH:mm"),
    ...
    getChartData() {
      ...
      list_in_storage_time({
        ...
        if (res.data.success === true) {
          let date = res.data.data.map((x) => x.durationScope);
          let data = res.data.data.map((x) => x.num);
          let option = { id: 'bdc3', x: date, nameX: '分钟', y: [data], name: ['在库时长'] }
