...
export default {
  ...
  methods: {
    ...
    changeFixData(index) {
      if (index == 0) {
        this.fixData = this.fixData100
      } else {
        this.fixData = this.fixData200
      }
    },
    ...
    getFixRecord() {
      ...
      // 停线记录接口
      downtime_record({ ...this.key, rowNum: 3, lineBody: 'SVW Anting MEB 200' }).then((res) => {
        if (res.data.success === true) {
          this.fixData200 = res.data.data.map((x) => ({
            ...x,
            sTime: moment(x.startTime).format("MM-DD HH:mm"),
            eTime: moment(x.endTime).format("MM-DD HH:mm"),
          }));
        }
      });
      downtime_record({ ...this.key, rowNum: 3, lineBody: 'SVW Anting MEB 100' }).then((res) => {
        if (res.data.success === true) {
          this.fixData100 = res.data.data.map((x) => ({
            ...x,
            sTime: moment(x.startTime).format("MM-DD HH:mm"),
            eTime: moment(x.endTime).format("MM-DD HH:mm"),
          }));
