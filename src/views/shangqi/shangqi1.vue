<template>
  <div class="home zl-body shangqi1">
    <div class="head-box">
...
import { stackEchartOption, pointOneChartFun, sanKeyOption, girdEchartOption, girdLineEchartOptions, yuanEchartOption, banyuanEchartOption, areaLineEchart, onePieEchartOption } from '@/utils/echartOption'
import { list_downtime_production_line, asrsImages, month_un_safety_production_day, capacityDaily, list_day_total_single_tce, list_month_tce, list_day_tce, list_month_total_single_tce, get_total_and_single_target, list_sankey_diagram, get_today_test_data } from '@/api/abcd'
...
export default {
  ...
  data() {
    return {
      ...
      feiQiPoint: [
        { name: "一线电泳烘房P1", obj: { jsCurrent: 0.117, dyCurrent: 31.236 }, key: '3', top: '29%', right: '12%', },
        { name: "一线电泳焙房P2", obj: { jsCurrent: 0.143, dyCurrent: 28.881 }, key: '2', top: '29.5%', right: '10.5%', },
        { name: "二线电泳焙房P1", obj: { jsCurrent: 1.461, dyCurrent: 13.368 }, key: '1', top: '30.3%', right: '9%', },
        { name: "二线电泳焙房P2", obj: { jsCurrent: 0.083, dyCurrent: 32.538 }, key: '6', top: '32%', right: '13.5%', },
        { name: "一线预凝胶烘房P1", obj: { jsCurrent: 0.108, dyCurrent: 1.764 }, key: '5', top: '32.5%', right: '12%', },
        { name: "一线预凝胶烘房P2", obj: { jsCurrent: 0.083, dyCurrent: 32.538 }, key: '4', top: '33.3%', right: '10.5%', },
        { name: "一线喷漆和中间烘房", obj: { jsCurrent: 0.005, dyCurrent: 33.634 }, key: '12', top: '34.5%', right: '24%', },
        { name: "二线喷漆和中间烘房", obj: { jsCurrent: 0.228, dyCurrent: 24.136 }, key: '11', top: '35.2%', right: '22.8%' },
        { name: "三线喷漆和中间烘房", obj: { jsCurrent: 0.096, dyCurrent: 19.232 }, key: '10', top: '36%', right: '21.5%' },
        { name: "一线面漆烘房", obj: { jsCurrent: 1.248, dyCurrent: 7.395 }, key: '9', top: '39%', right: '26.5%' },
        { name: "二线面漆烘房", obj: { jsCurrent: 2.733, dyCurrent: 7.037 }, key: '8', top: '41%', right: '23.5%' },
        { name: "三线面漆烘房", obj: { jsCurrent: 9.124, dyCurrent: 6.534 }, key: '7', top: '40%', right: '25%' },
      ],
      waterPoint: [
        { name: "工厂总排口", obj: {}, key: '3', top: '61%', right: '38%', },
        { name: "废水总排口", obj: {}, key: '3', top: '59%', right: '24%', },
      ]
    }
  ...
  methods: {
      ...
      if (this.shxsModel == '日') {
        capacityDaily().then(res => {
          if (res.data.success) {
            ...
            type.forEach(item => {
              let arr = res.data.data.filter(v => item == v.kindZZ)
              let newArr = []
              arr.forEach(a => {
                if (date.indexOf(a.theDate) > -1) {
                  newArr[date.indexOf(a.theDate)] = a.pdNums
                }
              })
              console.log(newArr)
              data.push(newArr)
    ...
    changeNyhj() {
      if (this.nyhjChange == '月') {
        // 总能
        list_month_total_single_tce({ isSingle: 0, rowNum: 7 }).then((res) => {
          if (res.data.success) {
            // debugger

            let data = res.data.data.map((x) => x.tce);
            let date = res.data.data.map((x) => moment(x.statisDate).format('yyyy-MM'));
        ...
        // 单能
        list_month_total_single_tce({ isSingle: 1, rowNum: 7 }).then((res) => {
          if (res.data.success) {
            // debugger
    ...
    getObjData() {
      ...
      completeRate({ ...this.key }).then((res) => {
        if (res.data.success === true) {
          ...
          tihs.completeRateNum = res.data.data
        }
      });
      ...
      list_sankey_diagram().then((res) => {
        if (res.data.success === true) {
          sanKeyOption({ id: 'sankey1', f: res.data.data, fontSize: 12, hiddenLabel: true });
      ...
      //   }
      // });
      get_total_and_single_target().then((res) => {
        if (res.data.success === true) {
          // this.obj0 = {...res.data.data}
          let data = res.data.data || {};
          let tce = data.tce;
          pointOneChartFun(res.data.data.totalConsumptionRatio, 100, 'pointOneChart')
          pointOneChartFun(res.data.data.singleConsumptionRatio, 100, 'pointOneChart1')
        }
      });
    },
    loadChart() {
...
.shangqi1 .el-tree-node__expand-icon {
  color: white;
}
.shangqi1 .el-tree-node__expand-icon.is-leaf {
  opacity: 0;
}
.shangqi1 .el-tree-node:focus > .el-tree-node__content {
  background-color: #66b1ff87 !important;
}
.shangqi1 .el-tree-node__content:hover {
  background-color: #66b1ff87 !important;
}
.shangqi1
...
//   }
// }
.shangqi1 .szrs .el-select-dropdown__item.hover {
  background-color: #fff;
  color: #000;
}
.shangqi1 .szrs.el-select .el-select-dropdown {
  background-color: #152940;
  border: 1px solid #4ec4f8;
  color: #fff;
}
.shangqi1 .szrs.el-select li {
  display: flex;
  align-items: center;
  justify-content: center;
}
.shangqi1 .szrs .el-select-dropdown__item {
  color: #ccc;
}
</style>
